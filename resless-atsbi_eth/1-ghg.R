...
dmi_ct<-((fw_g_ct*dm_g)+(fw_rc_ct*dm_rc)+(fw_rl_ct*dm_rl)+(fw_pf_ct*dm_pf)+(fw_conc_ct*dm_conc)+(fw_conos_ct*dm_conos)+(fwg_hay_ct*dm_h)+(fwg_sil_ct*dm_s))
...
dmi_sh<-((fw_g_sh*dm_g)+(fw_rc_sh*dm_rc)+(fw_rl_sh*dm_rl)+(fw_pf_sh*dm_pf)+(fw_conc_sh*dm_conc)+(fw_conos_sh*dm_conos)+(fwg_hay_sh*dm_h)+(fwg_sil_sh*dm_s))
...
lcide_ct <- (perc_dm_g_ct*d_g)+(perc_dm_rc_ct*d_rc)+(perc_dm_rl_ct*d_rl)+(perc_dm_pf_ct*d_pf)+(perc_dm_conc_ct*d_conc)+(perc_dm_conos_ct*d_conos)+(perc_dm_sil_ct*d_s)+(perc_dm_h_ct*d_h)
...
lcide_sh <- (perc_dm_g_sh*d_g)+(perc_dm_rc_sh*d_rc)+(perc_dm_rl_sh*d_rl)+(perc_dm_pf_sh*d_pf)+(perc_dm_conc_sh*d_conc)+(perc_dm_conos_sh*d_conos)+(perc_dm_sil_sh*d_s)+(perc_dm_h_sh*d_h)
# methane conversion factor, constants are taken from (tier 2 estimation) Gerber et al 2011
...
fun<-function(x){x*rum_co2e_yeartot_ct/sumlivdist}
rum_co2e_map_ct<-overlay(livdist,fun=fun)
fun<-function(x){x*rum_co2e_yeartot_sh/sumlivdist}
...
fun <-function (a,b,c,d,f,g,h,i ) {vs_ct*365*B0*0.67 * ((a/100*mms_lagoon_perc/100)+
...
efc1<- overlay (mms_lagoon_mcf, mms_liquidslurry_mcf, mms_solidstorage_mcf ,mms_drylot_mcf,mms_dailyspread_mcf,mms_digester_mcf,mms_burned_mcf,mms_other_mcf, fun=fun )


fun1<-function(a,b){a*(numcow_es+numcow_is)/ncellcrop*b }
...
fun <-function (e,d ) {(vs_ct*365*B0*0.67 *(e/100*mms_pasture_perc/100))*d}
efg1 <- overlay (mms_pasture_mcf,grazland, fun=fun )

#spatial allocation
fun<- function(a,c){a*(numcow_es+numcow_is)/ncellgraz*c}
manure_CH4_year_herd_g1 <- overlay (efg1,grazland, fun=fun )

#total
fun=function(x,y) {x+y}
manure_CH4_year_herd_tot1 <-overlay(manure_CH4_year_herd_g1, manure_CH4_year_herd_c1, fun=fun)
#plot(manure_CH4_year_herd_tot)

# other cattle
fun <-function (a,b,c,d,f,g,h,i ) {vs_ct*365*B02*0.67 * ((a/100*mms_lagoon_perc2/100)+
...
efc2<- overlay (mms_lagoon_mcf, mms_liquidslurry_mcf, mms_solidstorage_mcf ,mms_drylot_mcf,mms_dailyspread_mcf,mms_digester_mcf,mms_burned_mcf,mms_other_mcf, fun=fun )


fun1<-function(a,b){a*(numcow_sis+numcow_da)/ncellcrop*b }
manure_CH4_year_herd_c2<-overlay(efc2,cropland, fun=fun1)


# sheep
fun <-function (a ) {vs_sh*365*B03*0.67 * a}
efsh<- overlay (mcf_sh, fun=fun )

fun1<-function(a,b){a*(numcow_sis+numcow_da)/ncellcrop*b }
manure_CH4_year_herd_c2<-overlay(efc2,cropland, fun=fun1)
fun <-function (e,d ) {(vs_ct*365*B0*0.67 *(e/100*mms_pasture_perc2/100))*d}
efg2 <- overlay (mms_pasture_mcf2,grazland, fun=fun )

#spatial allocation
fun<- function(a,c){a*(numcow_sis+numcow_da)/ncellgraz*c}
manure_CH4_year_herd_g2 <- overlay (efg2,grazland, fun=fun )
fun=function(x,y) {x+y}
manure_CH4_year_herd_tot2 <-overlay(manure_CH4_year_herd_g2, manure_CH4_year_herd_c2, fun=fun)
#plot(manure_CH4_year_herd_tot)
...
## total 
manure_CH4_year_herd_tot <- manure_CH4_year_herd_tot1 +manure_CH4_year_herd_tot2 + manure_CH4_year_herd_sh
...
#for dairy
Nrate1 <- NrateLook$ExcretionRate[NrateLook$Species==species & NrateLook$Region==region]
Nrate2 <- NrateLook$ExcretionRate[NrateLook$Species==species3 & NrateLook$Region==region]
Nrate3 <- NrateLook$ExcretionRate[NrateLook$Species==species5 & NrateLook$Region==region]
...
#Direct
#dairy
N2O_d_year_herd1 <- sum(Nex1*mms_lagoon_perc/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="UncoveredAnerobic"] , 
                         Nex1*mms_liquidslurry_perc/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="Liquid"],
                         Nex1*mms_solidstorage_perc/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="SolidStorage"],
                         Nex1*mms_drylot_perc/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="DryLot"],
                         Nex1*mms_dailyspread_perc/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="DailySpread"],
                         Nex1*mms_digester_perc/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="AnerobicDigester"],
                         Nex1*mms_other_perc/100 * MMS_N_EFmean)*(44/28) 
#44/28 = conversion of (N2O-N)(mm) emissions to N2O(mm) emissions

#other cattle
N2O_d_year_herd2 <- sum(Nex2*mms_lagoon_perc2/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="UncoveredAnerobic"] , 
...
#sheep
N2O_d_year_herd3 <- sum( Nex3*(100-mms_pasture_perc3)/100 * MMS_N_EF$EF[MMS_N_EF$MMS=="SolidStorage"])*(44/28) 
...
N2Odeposit_indirect_ct <- ((Ndeposit1+Ndeposit2 )* FracGASM * EF_indirectN2Opasture)*(44/28) #Equation 11.9 IPCC
N2Odeposit_indirect_sh <- (Ndeposit3 * FracGASM * EF_indirectN2Opasture)*(44/28) #Equation 11.9 IPCC
...
N_vol_herd1 <- sum(Nex1*mms_lagoon_perc/100 * N_loss1$N_loss[ N_loss1$MMS=="UncoveredAnerobic"] , 
                    Nex1*mms_liquidslurry_perc/100 * N_loss1$N_loss[N_loss1$MMS=="Liquid"],
                    Nex1*mms_solidstorage_perc/100 * N_loss1$N_loss[ N_loss1$MMS=="SolidStorage"],
                    Nex1*mms_drylot_perc/100 * N_loss$N_loss1[ N_loss1$MMS=="DryLot"],
                    Nex1*mms_dailyspread_perc/100 * N_loss1$N_loss[N_loss1$MMS=="DailySpread"],
                    Nex1*mms_other_perc/100 * N_lossmean1)
...
N_vol_herd2 <- sum(Nex2*mms_lagoon_perc2/100 * N_loss2$N_loss[ N_loss2$MMS=="UncoveredAnerobic"] , 
                   Nex2*mms_liquidslurry_perc2/100 * N_loss2$N_loss[N_loss2$MMS=="Liquid"],
                   Nex2*mms_solidstorage_perc2/100 * N_loss2$N_loss[ N_loss2$MMS=="SolidStorage"],
                   Nex2*mms_drylot_perc2/100 * N_loss2$N_loss[ N_loss2$MMS=="DryLot"],
                   Nex2*mms_dailyspread_perc2/100 * N_loss2$N_loss[N_loss2$MMS=="DailySpread"],
                   Nex2*mms_other_perc2/100 * N_lossmean2)
N2O_i_year_herd2 <-((N_vol_herd2) *0.01)*(44/28)

N_vol_herd3 <- sum(Nex2*(100-mms_pasture_perc3)/100 * N_loss3$N_loss[ N_loss3$MMS=="SolidStorage"])
N2O_i_year_herd3 <-((N_vol_herd3) *0.01)*(44/28)

fun<-function(x){((N2O_i_year_herd1+N2O_d_year_herd1+N2O_i_year_herd2+N2O_d_year_herd2)/ncellcrop)*(x)} 
N2O_t_year_herd_map_c_ct<- overlay(cropland,fun=fun )
fun<-function(x){((N2O_i_year_herd3+N2O_d_year_herd3)/ncellcrop)*(x)} 
N2O_t_year_herd_map_c_sh <- overlay(cropland,fun=fun )
N2O_t_year_herd_map_c<-N2O_t_year_herd_map_c_sh+N2O_t_year_herd_map_c_ct


fun<-function(x){((N20deposit_direct1+N2Odeposit_indirect_ct)/ncellgraz)*x} 
N2O_t_year_herd_map_g_ct <- overlay(grazland,fun=fun )
fun<-function(x){((N20deposit_direct2+N2Odeposit_indirect_sh)/ncellgraz)*x} 
...
N2O_t_year_herd_map_ct <- overlay(N2O_t_year_herd_map_c_ct,N2O_t_year_herd_map_g_ct,fun=fun )
N2O_t_year_herd_map_sh <- overlay(N2O_t_year_herd_map_c_sh,N2O_t_year_herd_map_g_sh,fun=fun )
...
co2e_manure_ct<-overlay(N2O_t_year_herd_map_ct,manure_CH4_year_herd_ct,fun=fun)
co2e_manure_sh<-overlay(N2O_t_year_herd_map_sh,manure_CH4_year_herd_sh,fun=fun)
...
fun=function(w,x,y,z){w+x+(fw_pf_ct+fw_rc_ct+fw_rl_ct)/(fw_pf+fw_rc+fw_rl) * y+z} #cslucs is not assigned, because it is always zero for this case
COe_ct<-overlay(rum_co2e_map_ct,co2e_manure_ct,scb,csluc,fun=fun)

fun=function(w,x,y,z){w+x+(fw_pf_sh+fw_rc_sh+fw_rl_sh)/(fw_pf+fw_rc+fw_rl) * y+z}
...
co2meat<-round((COe_l_ct*(numcow_sis+numcow_da)/numcow +COe_l_sh)/(meat),0)

co2meat_ct<-round((COe_l_ct*(numcow_sis+numcow_da)/numcow)/(cattle_meat),0)
co2meat_sh<-round(COe_l_sh/sheep_meat,0)


rum_co2e_yeartot<-round(rum_co2e_yeartot,0)
Coe_ind<-data.frame(COe_l, COe_l_ct, COe_l_sh,co2cow,co2sh,co2milk,co2meat)
...
names(Coe_ind2)<-c("CO2 emmissions", "CO2 emmissions Cattle","CO2 emmissions Sheep","CO2 per cow","CO2 per sheep" ,"CO2 per tonnes of milk","CO2 per tonnes of meat")
