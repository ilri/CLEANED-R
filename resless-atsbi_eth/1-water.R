...
fun=function(x,y,z,v){(ar_rc/(ar_rc+ar_rl+ar_pf))*x+(ar_rl/(ar_rc+ar_rl+ar_pf))*y+(ar_pf/(ar_rc+ar_rl+ar_pf))*z+ v}
...
wu_cow<-round(wr_sum*(fw_pf_ct+fw_rc_ct+fw_rl_ct+fw_g_ct)/(fw_pf+fw_rc+fw_rl+fw_g)/numcow,0) #num cow here takes the average of the pastoral cows over the year
wu_sheep<-round(wr_sum*(fw_pf_sh+fw_rc_sh+fw_rl_sh+fw_g_sh)/(fw_pf+fw_rc+fw_rl+fw_g)/numsheep,0) #num cow here takes the average of the pastoral cows over the year

wratio<-round((wr_sum/(cellStats(rain, stat='sum'))), 2) # in cubic meters

# why not per produce 
wu_milk<-round(wu_cow*(numcow_es+numcow_is/numcow)/milk,0) 
wu_meat<-round((wu_cow*(numcow_sis+numcow_da/numcow)+wu_sheep)/meat,0) 
...
water_ind<-data.frame(wr_sum, wratio, wu_cow,wu_sheep,wu_milk,wu_meat,wui_avg )
...
names(water_ind2)<-c("total water consumption"," water consumption rainfall ratio", 'water consumption per cow', 'water consumption per sheep',  'water consumption per ton of milk',  'water consumption per ton of meat',"average water consumption intensity")
