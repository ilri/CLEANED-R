...
path<-'D:/Dropbox/CLEANED - IMPACT-BFA'
setwd(path)
...
IMPscen <- 'OptDemand' # 'PesDemand', 'OptDemand','BaseDemand' 
...
add<- 0 # in percent, will be devided by 100 in the  luc code additional biomass
corrOption<- 1 # 1=cropland only 2= newland is agro-forestry 
nopgc <- 0 # if 1 then no crop productivity gain is assumed, make sure that you adjust pgc with base run in the if loop 
#give your scanario a name 

name<-'OptDemand' #Define scenario here
#add<- add+add_cc
# AMT-Beef
# AMT-Pork
# AMT-Lamb
...
ipcc= 1 # for IMPACT leave it at 1
...
#import_c*0.001 /(biomass2-673266)*100
