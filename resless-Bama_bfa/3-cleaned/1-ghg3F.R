######################### greenhouse gases  pathway Burkina Faso ##############
...
dmi<-((fw_g*dm_g)+(fw_rc*dm_rc)+(fw_rl*dm_rl)+(fw_pf*dm_pf)+(fw_conc*dm_conc)+(fw_conos*dm_conos)+(fw_rr*dm_rr))
...
fun1<-function(a,b,c,d){(a*(numcow_d+numcow_tl)+b*(numcow_tpt+numcow_f+numcow_da+numcow_tt*st1/12))/(ncellcrop+ncellrice)*(c+d)}
manure_CH4_year_herd_c<-overlay(efc_d,efc_c,cropland, riceland, fun=fun1)
...
#CH4 estimates
fun <-function (e ) {(vs*365*B0*0.67 *(e/100*mms_pasture_perc/100))*(numcow_d+numcow_tl)}
efg_d <- overlay (mms_pasture_mcf, fun=fun )

fun <-function (e ) {(vs*365*B02*0.67 *(e/100*mms_pasture_perc2/100))*(numcow_tpt+numcow_f+numcow_da+numcow_tt*st1/12)}
efg_c <- overlay (mms_pasture_mcf, fun=fun )

fun<- function(a,b,c){(a+b)/ncellgraz*c}
manure_CH4_year_herd_g <- overlay (efg_c,efg_d,grazland, fun=fun )
...
Nex <- Nrate*((lwis*numcow_d+lwles*numcow_tl)/1000)*365 #for all dairy animals 
Nex2 <- Nrate2*((lwles*numcow_tpt+lwsis*numcow_f+lwda*numcow_da)/1000)*365 + Nrate2*(lwes/1000)*numcow_tt*365*st1/12 # for all other cattle
...
N2O_d_year_herd<-N2O_d_year_herd_d+N2O_d_year_herd_c


# Pasture (grazed and deposited) calculated is now calculted here 

#Direct and indirect N2O on pastures
Ndeposit <- (Nex+Nex2)*(mms_pasture_perc/100) #Equation 11.5 IPCC
...
co2milk<-round(COe_l/(milk_les+milk_is),0)
co2meat<-round(COe_l/(meat_es+meat_sis), 0 )
...
names(Coe_ind2)<-c("CO2 émmis", "CO2 issu du fumier", "CO2 issu de fermentation entérique","CO2 émmis par tête", "CO2 émmis par tonne de lait", "CO2 émmis par tonne de viande" )
Coe_ind2<-data.frame(t(Coe_ind2))
colnames(Coe_ind2)<-c('resultat','difference','Pourcent')

##steve edits
Coe_ind_val<-read.csv('1-input/parameter/ghg_val.csv')
Coe_ind2$val <- ifelse(Coe_ind2$Pourcent <= Coe_ind_val$peu, 'faible', ifelse((Coe_ind2$Pourcent >= Coe_ind_val$peu) & (Coe_ind2$Pourcent <= Coe_ind_val$peu),'moyen','important'))
