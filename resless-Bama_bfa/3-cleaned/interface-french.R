...
sidebar1<-dashboardSidebar(width=150,
                 sidebarMenu(
                   menuItem(""),
                   menuItem("A PROPOS DE :", tabName = "widgets1", icon = icon("th")),
                   menuItem("VERSION SIMPLE: ", tabName = "dashboard1", icon = icon("dashboard")),
                   menuItem("VERSION EXPERT :", tabName = "widgets1a", icon = icon("th"))
...
box(width=12,background ="green", 
  ...
  img(src="BuksF.jpg", width=650))
           
)
    ),
fluidRow(box(width=12, background ="blue",title="Zone d'etude: Bama, zone periurbaine de Bobo Dioulasso, Burkina Faso",
             column(width=1,offset=1,align="center",
          img(src="studyarea_FR.jpg", width=650))),
box(width=12,background ="green", 
column(width=1,offset=1,align="center" ,
             img(src="Buks2F.jpg", width=650)
         )
         )),
...
box(width=12,background ="black", title="Links",
    fluidRow(column(width=1,offset=1, h4("", style="padding:10px;")),
             url <- a("Clickez ici pour le site web de ILRI ", href="https://www.ilri.org/")),
    fluidRow(column(width=1,offset=1, h4("", style="padding:10px;")),
             url <- a("Clickez ici pour le site web de SEI", href="https://www.sei.org")),
    fluidRow(column(width=1,offset=1, h4("", style="padding:10px;")),
             url <- a("Clickez ici pour le site web de LocateIT ", href="http://www.locateit.co.ke")),
    fluidRow(column(width=1,offset=1, h4("", style="padding:10px;")),
             url <- a("Clickez ici pour le site web de GITHUB (link code) ", href="https://github.com/pfeiferc/CLEANED-R/tree/master/Bama-BurkinaFaso")),
    fluidRow(column(width=1,offset=1, h4("", style="padding:10px;")),
             url <- a("Clickez ici pour la documentation de l'outil CLEANED", href="https://cgspace.cgiar.org/bitstream/handle/10568/93076/sairla_CLEANED.pdf?sequence=1&isAllowed=y")),
    fluidRow(column(width=1,offset=1, h4("", style="padding:10px;")),
             url <- a("Clickez ici pour d'autres publications", href="https://cgspace.cgiar.org/handle/10568/33745"))
...
tabItem(tabName="widgets1a",
        ...
        url <- a("Cliquez ici pour la version experte de l'outil CLEANED", href="https://ilri.shinyapps.io/Advancedcleanedtool/")
...
#),
  ...
  mainPanel(
    ...
    tabsetPanel(id = "inTabset",
                        ...
                        tabPanel(title = "Impact eau", value = "panel2",
                                 fluidRow(box(width=12, background ="blue",img(src="waterimpact.jpg", width=500))),
                                 fluidRow(box(width=12, title = "", height=700,
                                              actionButton("gowt", "Calculer l'impact eau"),
                                            
                                              plotOutput("plot12"))),
                                 
                                 fluidRow(box(width=12, title = "Resultats eau",
                                              
                                            
                                              plotOutput("plotable"))
                                 )
                        ),
                        tabPanel(title = "Impact gas a effet de serre", value = "panel3", 
                                 fluidRow(box(width=12, background ="green",img(src="greenhouseimpact.jpg", width=500))),
                                 fluidRow(box(width=12, title = " ",height=700,
                                              actionButton("gogh", "Calculer l'impact gas a effet de serre"),
                                              # numericInput("n", "n", 50),
                                              plotOutput("plot123"))),
                                 fluidRow(box(width=12, title = "Resultats gas a effet de serre",
                                              #actionButton("gowt", " "),
                                              # numericInput("n", "n", 50),
                                              plotOutput("plotableghg"))
                                 )),
                        tabPanel(title = "Impact biodiversite", value = "panel4", 
                                 fluidRow(box(width=12, background ="light-blue",img(src="biodiversityimpact.JPG", width=500))),
                                 fluidRow(box(width=12, title = " ",height=700,
                                              actionButton("gobdiv", "Calculer l'impact biodiversite"),
                                              # numericInput("n", "n", 50),
                                              plotOutput("plot12bid"))
                                          
                                 ),
                                 
                                 fluidRow(box(width=12, title = "Resultats biodiversite",
                                              #actionButton("gowt", " "),
                                              # numericInput("n", "n", 50),
                                              plotOutput("plotablediv"))
                                 )),
                        tabPanel(title = "Impact sol", value = "panel5", 
                                 fluidRow(box(width=12, background ="red",img(src="soilimpact.JPG", width=500))),
                                 fluidRow(box(actionButton("gosol", "Calculer l'impact sol",icon = NULL), title = " ",height=1000, width=12,
                                  ...
                                  fluidRow(box(width=12, title = "Resulats impact sol",
                                              #actionButton("gowt", " "),
                                               # numericInput("n", "n", 50),
                                               plotOutput("plotablesoil")))
                                 )
  ...
  box(#status = "warning", title=" ", width = 12, "", background ="red",
    HTML('<script type="text/javascript">
        $(document).ready(function() {
         ...
         '),
      div(style="height: 35px;", actionButton("go", "Clickez ici pour mettre a jour les donnees de production ")) ,width = 12, background ="red" )
...
fluidRow(
  box(status = "warning",width = 2, "",background ="red", textInput("name","donnez un nom a votre scenario:","LeMien")),
  box(status = "warning", width = 2, "TOUPEAUX PASTORAUX TRANSHUMANTS",background ="maroon"),
  box(status = "warning", width = 2,"TROUPEAUX LAITIERS PASTORAUX",background ="maroon"),
  box(status = "warning", width = 2, "ANIMAUX LAITIERS",background ="maroon"),
  box(status = "warning", width = 2, "ANIMAUX D'EMBOUCHE",background ="maroon"),
  box(status = "warning", width = 2, "ANIMAUX DE TRAIT",background ="maroon")),
  


fluidRow(
  box(status = "warning", width = 2, "", background ="green",
      div(style="height: 35px;",textInput("1a", "", value="No. de troupeaux")),
      div(style="height: 35px;",textInput("2a", "",value="No. d'ani./troup")),
      textInput("3a", "",value="No.d'animaux")
   
      
  ), 

  box(status = "warning", width = 1, "",background ="green",
      div(style="height: 35px;;font-size:12px;",tags$style("#tt {font-size:12px;}"),numericInput("tt", " grandeT", 100, min = 60, max =1000000)),
  ...
  box(status = "warning", width = 1, "",background ="green",
      div(style="height: 35px;",numericInput("tpt", " petiteT", 238, min = 60, max =1000000)),
...
numericInput("numcow_c", "No. d'animaux tranversant la zone:", 200000, min = 10, max =1000000 ))),

fluidRow(
  box(status = "primary",width = 12, " SELECTIONNEZ DES SCENARIOS  :-----------:-----------:------------:-------------:------:-----:-----------: ",background = "navy"),
 sidebarPanel(
    tabsetPanel(id = "tabset",
                ...
                tabPanel("changement d'utilisation du sol",
...
fluidRow(
  box(width=6, " Productivite",plotOutput("baseplot")))
...
ui <- dashboardPage(
  dashboardHeader(title = "PROJET ReSLeSS  : OUTIL CLEANAED POUR BAMA, BURKINA FASO",titleWidth=750,
  ...
  observeEvent(input$hideSidebar, {
   ...
   pok<- eventReactive(input$go, {
    ...
    lwes<<-200
     ...
     source('2-feedbasket_nonlinear2F2.R')
   
     
     paste("Parameteres de production sont mis a jour")
...
output$plot <- renderText(
    ...
    { 
      withProgress(pok(), message="Attendez SVP : calculation de la production.")
 ...
 setwd(path)
  ...
  output$plot12 <- renderPlot(
    { 
      withProgress(pok1(), message="Attendez SVP : calculation de l'impact eau" )
  ...
  output$plot123 <- renderPlot(
    { 
      ...
      withProgress(pok1gh(), message="Attendez SVP : calculation de l'impact gas a effet de serre" )
      pok1gh()
  ...
  output$plot12bid <- renderPlot(
    { 
      ...
      withProgress(pok1bdiv(), message="Attendez SVP : calculation de l'impact biodiversite." )
 ...
 setwd(path)
  ...
  output$plotsoil <- renderPlot(
    { 
      ...
      withProgress(psoil(), message="Attendez SVP : calculation de l'impact sol" )
