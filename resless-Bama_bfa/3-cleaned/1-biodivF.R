...
names(bio_ind2)<-c("index de richesse moyen", "nbr moyen d'espèces critiques perdant l'habitat")
bio_ind2<-data.frame(t(bio_ind2))
colnames(bio_ind2)<-c('resultat','difference','Pourcent')

##steve edits
bio_ind_val<-read.csv('1-input/parameter/bio_val.csv')
bio_ind2$val <- ifelse(bio_ind2$Pourcent <= bio_ind_val$peu, 'faible', ifelse((bio_ind2$Pourcent >= bio_ind_val$peu) & (bio_ind2$Pourcent <= bio_ind_val$peu),'moyen','important'))


title1<-paste('Impact biodiversité : index de richesse', name, sep=" ")
title2<-paste("Impact biodiversité : nombre d'espèce critiques" , name, sep=" ")
title3<-paste('Impact biodiversité : difference index de richesse', name, sep=" ")
