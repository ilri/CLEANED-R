...
if(preset==1){
    ...
    {
        assign(as.character(temp$Avar[i]), temp$ppara[i], envir=.GlobalEnv)
    }}
...
if(preset==1){  
    ...
    {
        assign(as.character(temp$Lvar[i]), temp$ppara[i], envir=.GlobalEnv)
    }}
...
if(preset==1){
    ...
    {
        assign(as.character(temp$Mvar[i]), temp$ppara[i], envir=.GlobalEnv)
    }}
...
if(preset==1){
    ...
    {
        assign(as.character(temp$Fvar[i]), temp$ppara[i], envir=.GlobalEnv)
    }}
...
if(preset==1){
    ...
    {
        assign(as.character(temp$Tvar[i]), temp$ppara[i], envir=.GlobalEnv)
    }}
...
names(prod_ind2)<-c('viande (tonnes)','lait (tonnes)', 'cultures(km2)', 'pâturages(km2)','riz (km2)','cultures utlisées(km2)','pâturages utilisés', 'riz utilisé', 'import cultures', 'import pâturages', 'import riz', 'nbr tête total', 'nbr tête longue transhumance', 'nbr tête petite transhumance', 'nbr tête laitier pastoral', 'nbr tête laitier specialize','nbr tête embouche', 'nbr tête trait')
prod_ind2<-data.frame(t(prod_ind2))
colnames(prod_ind2)<-c('resultat','difference','Pourcent')

##steve edits
prod_ind_val<-read.csv('1-input/parameter/productivity_val.csv')
prod_ind2$val <- ifelse(prod_ind2$Pourcent <= prod_ind_val$peu, 'faible', ifelse((prod_ind2$Pourcent >= prod_ind_val$peu) & (prod_ind2$Pourcent <= prod_ind_val$peu),'moyen','important'))
