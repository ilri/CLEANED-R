...
erc_pg<-(5.881255274+0.27415641*(lwpg))*0.0041868 *365# see pig excel 
erc_ch<- 2800*lwch*0.0041868 *365# http://www.ag.auburn.edu/~chibale/an12poultryfeeding.pdf this is ME requiremeent
# 0.0041868 to convert cal to mj 
#total energy requirement from ruminants 
erc<-erc_is+erc_sis+erc_da+erc_sg
##############################################################################################
#calculate energy requirement in by season  from each feed source 

#total energy required from each source of food
ng<-(sfng1/100*numcow_f*erc_sis + ifng1/100 * numcow_d*erc_is + dafng1/100 * (numcow_da*erc_da)+ sgfng1/100 * (num_sg*erc_sg))
rc<- ( sfrc1/100*numcow_f*erc_sis + ifrc1/100 * numcow_d*erc_is+ dafrc1/100 *(numcow_da*erc_da)+ sgfrc1/100 * (num_sg*erc_sg))
rr<- (sfrr1/100*numcow_f*erc_sis + ifrr1/100 * numcow_d*erc_is+ dafrr1/100 * (numcow_da*erc_da)+ sgfrr1/100 * (num_sg*erc_sg))
rl<-(sfrl1/100*numcow_f*erc_sis + ifrl1/100 * numcow_d*erc_is+ dafrl1/100 *(numcow_da*erc_da)+ sgfrl1/100 * (num_sg*erc_sg))
pf<-(sfpf1/100*numcow_f*erc_sis + ifpf1/100 * numcow_d*erc_is+ dafpf1/100 *(numcow_da*erc_da)+ sgfpf1/100 * (num_sg*erc_sg))
conc<-(sfconc1/100*numcow_f*erc_sis + ifconc1/100 * numcow_d*erc_is+ dafconc1/100 *( numcow_da*erc_da)+ sgfconc1/100 * (num_sg*erc_sg))
conos<-(sfconos1/100*numcow_f*erc_sis + ifconos1/100 * numcow_d*erc_is+ dafconos1/100 *(numcow_da*erc_da)+ sgfconos1/100 * (num_sg*erc_sg))
...
fw_cer_mono<-( num_pg*erc_pg)/(meconc_ch)+ (num_ch*erc_pg)/(meconc_pg)
...
meat_is<-(numcow_d*lwis*dis)*0.001
...
#area required 
ar_rc1<-fw_rc/avg_rc *pixel^2/1000000 # number pixel * adjustement in km2
ar_cer<-fw_cer/avg_cer
...
if (diff_ar>0){
     import_c = 0
     totmaize= cellStats(y_maizec*(1+pgc),stat='mean')*ar_rc1*0.001+  cellStats(y_maizec*(1+pgc),stat='mean')*(diff_ar)*0.001 - fw_cer*0.001 #fw_cer is in kg *0.001 to tons 
     #ar_rc1 is crop residue only, and ar_rc is the max area from ar_rc or ar_cer
} else {import_c= -diff_ar

     totmaize= cellStats(y_maizec*(1+pgc),stat='mean')*ar_rc1*0.001+  cellStats(y_maizec*(1+pgc),stat='mean')*(diff_ar)*0.001 - fw_cer*0.001  #fw_cer is in kg
     #ar_rc1 is crop residue only, and ar_rc is the max area from ar_rc or ar_cer
...
prod_ind<-data.frame( meat, milk , totmaize, croparea,grazarea, ar_rc, ar_g ,import_c, import_g  , numcow, numcow_d, numcow_f, numcow_da )
...
names(prod_ind2)<-c("meat produced","milk produced", "maize for consumption", 'total area available for crop', 'total area available for pasture',  "crop area used", 'pasture area used',  "import crop", 'import pasture', 'total numbers of cows',   'dairy animals', 'meat animals', 'follower animals' )
